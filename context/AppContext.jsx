...
import { hardcodedProducts } from "@/assets/hardcodedProducts";
  ...
  const fetchProductData = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const hardcodedMap = new Map(hardcodedProducts.map(p => [p._id, p]));
        const apiMap = new Map(apiProducts.map(p => [p._id, p]));
        
        // Combine: API products first, then hardcoded products that aren't in API
        const combined = [
          ...apiProducts,
          ...hardcodedProducts.filter(p => !apiMap.has(p._id))
        ];
      ...
      // If API fails, use hardcoded products
      setProducts(hardcodedProducts);
      console.error("Error fetching products, using hardcoded:", error);
    }
    setIsLoading(false);
  };
